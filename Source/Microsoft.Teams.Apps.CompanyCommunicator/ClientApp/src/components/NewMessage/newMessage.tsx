...
> {
  ...
  constructor(props: INewMessageProps) {
    ...
    this.state = {
      dynamicState: [{ id: 0, buttonTitle: "", buttonUrl: "" }],
      title: "",
      summary: "",
      author: "",
      btnLink: "",
  ...
  private getItem = async (id: number) => {
    try {
      ...
      const selectedRadioButton =
        draftMessageDetail.rosters.length > 0
          ? "rosters"
          : draftMessageDetail.allUsers
          ? "allUsers"
          : "teams";
  ...
  public onButtonTitleChanged = (id: number, value: any): void => {
    const dynamicState = [...this.state.dynamicState];
    dynamicState[id].buttonTitle = value;
    this.setState({ dynamicState });
  };
  public onButtonLinkChanged = (id: number, value: any): void => {
    const dynamicState = [...this.state.dynamicState];
    dynamicState[id].buttonUrl = value;
    this.setState({ dynamicState });
  };

  public _addButton = (): void => {
    const dynamicState = [...this.state.dynamicState];
    dynamicState.push({
      id: dynamicState.length,
      buttonTitle: "",
      buttonUrl: ""
    });
    this.setState({ dynamicState });
          ...
          <div className="taskModule">
            <div className="formContainer">
              ...
              {this.state.dynamicState &&
                this.state.dynamicState.map((item, index) => {
                  return (
                    <>
                      <DynamicButtons
                        key={index}
                        ...
                        onButtonTitleChanged={this.onButtonTitleChanged}
                        onButtonLinkChanged={this.onButtonLinkChanged}
                      />
                    </>
                  );
                })}
              <Button
                primary
                content="Add Button"
                disabled={this.state.dynamicState.length === 4}
              ...
              <div className="formContentContainer">
                ...
                >
                  <Radiobutton
                    name="grouped"
                    value="teams"
                    label="Send to General channel of specific teams"
                  />
                  <Dropdown
                  ...
                  <Radiobutton
                    name="grouped"
                    value="rosters"
                    label="Send in chat to specific people"
                  />
                  <Dropdown
                  ...
                  <Radiobutton
                    name="grouped"
                    value="allUsers"
                    label="Send in chat to everyone"
                  />
                  <div
                    className={
                  ...
                  >
                    <div className="noteText">
                      <Text
                        error
                        content="Note: This option sends the message to everyone in your org who has access to the app."
                      />
                    </div>
                  </div>
                </RadiobuttonGroup>
  ...
  private onGroupSelected = (value: any) => {
    this.setState({
      ...
      selectedRostersNum:
        value === "rosters" ? this.state.selectedRostersNum : 0
    });
  };
  ...
  private isSaveBtnDisabled = () => {
    ...
    return (
      !teamsSelectionIsValid || !rostersSelectionIsValid || nothingSelected
    );
  };
  ...
  private getItems = () => {
    ...
    if (this.state.teams) {
      ...
      if (this.state.selectedRadioBtn !== "allUsers") {
        remainingUserTeams =
          this.state.selectedRadioBtn === "teams"
            ? this.state.teams.filter(
                x =>
                  this.state.selectedTeams.findIndex(
                    y => y.team.id === x.teamId
                  ) < 0
              )
            : this.state.teams.filter(
                x =>
                  this.state.selectedRosters.findIndex(
                    y => y.team.id === x.teamId
                  ) < 0
              );
      }
